...
def ensemble_acc_dir(models_arch, models_name, models_path, PATH, weights=None):
    ...
    for idx, arch in enumerate(models_arch):
        ...
        preds, y = learn.predict_with_targs(is_test=False)
        print(models_name[idx], ':   ', accuracy_np(preds, y))
        acc_list.append(accuracy_np(preds, y))
        all_preds.append(preds)
        all_y.append(y)
    ...
    all_preds = np.array(all_preds)
    probs = np.average(np.exp(all_preds), axis=0, weights=weights)
    bagging_acc = accuracy_np(probs, all_y)
    print(bagging_acc)
    return bagging_acc, acc_list, int_acc_list